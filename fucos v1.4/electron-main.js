...
async function runApp() {
  ...
  try {
    // start server and get actual port (server will try alternate ports if 3000 is in use)
    const serverModule = require('./server');
    const chosenPort = await serverModule.startServer();
    SERVER_URL = `http://localhost:${chosenPort}`;
