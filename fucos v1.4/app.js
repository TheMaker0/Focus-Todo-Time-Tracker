  ...
  // Export all items with type 'ongoing' into the active sheet
  function exportOngoingToSheet() {
    try {
      ensureSheetBooks();
      const sheet = activeSheet();
      if (!sheet) return showToast('No active sheet found');

      const ongoingItems = (items || []).filter(i => i.type === 'ongoing');
      if (!ongoingItems.length) return showToast('No ongoing tasks to save');

      sheet.rows = sheet.rows || [];
      const now = Date.now();
      ongoingItems.forEach((it, idx) => {
        const rowId = now + idx;
        const row = {
          id: rowId,
          code: (it.title || '').slice(0, 120),
          department: it.account || '',
          note: it.desc || '',
          status: it.status || ''
        };
        sheet.rows.push(row);
        // Mark item as recorded so user can tell it was exported
        it.recordedAt = now;
        it.recordedTo = sheet.name || `Sheet ${currentSheetIndex + 1}`;
      });

      saveSheetData();
      saveAll();
      render();
      showToast(`✅ ${ongoingItems.length} ongoing saved to "${sheet.name}"`);
    } catch (err) {
      console.error('Export ongoing error:', err);
      showToast('Failed to export ongoing tasks');
    }
  }
