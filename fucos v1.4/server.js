...
async function startServer(preferredPort) {
  const basePort = Number(preferredPort || process.env.PORT || PORT) || PORT;
  let port = basePort;
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      ...
      console.log(message);
      if (!runningInElectron) openBrowser(`http://localhost:${port}`);
      return port;
    } catch (err) {
  ...
  throw new Error(`Could not bind server after ${maxAttempts} attempts starting at ${basePort}`);
}

// If run directly, start server now
